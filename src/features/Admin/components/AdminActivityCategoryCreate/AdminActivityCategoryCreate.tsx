...
const INITIAL_VIDEOS: VideoItem[] = [
  {
    id: 1,
    title: '얘도 마찬가지로 제목 길어지면 쩜쩜쩜 처리될거임 아 배고푸당 힝',
    owner: '주현지',
    generation: '25-26',
  },
  { id: 2, title: '서혜근의 PM 강의 1', owner: '서혜근', generation: '24-25' },
  { id: 3, title: '서혜근의 완벽하고 매끄러운 PM 강의 2', owner: '서혜근', generation: '24-25' },
  { id: 4, title: '영상 제목', owner: '주현지', generation: '25-26' },
  { id: 5, title: '영상 제목', owner: '주현지', generation: '25-26' },
  { id: 6, title: '영상 제목', owner: '주현지', generation: '25-26' },
  { id: 7, title: '영상 제목', owner: '주현지', generation: '25-26' },
  { id: 8, title: '영상 제목', owner: '주현지', generation: '25-26' },
  { id: 9, title: '영상 제목', owner: '주현지', generation: '25-26' },
  { id: 10, title: '영상 제목', owner: '주현지', generation: '25-26' },
  { id: 6, title: '영상 제목', owner: '주현지', generation: '25-26' },
  { id: 7, title: '영상 제목', owner: '주현지', generation: '25-26' },
  { id: 8, title: '영상 제목', owner: '주현지', generation: '25-26' },
  { id: 9, title: '영상 제목', owner: '주현지', generation: '25-26' },
...
const AdminActivityCategoryCreate: NextPage = () => {
  ...
  const [categoryName, setCategoryName] = useState('이서영 PM의 프로젝트 관리법');
  const [status, setStatus] = useState<'public' | 'private'>('public');
  const [videos, setVideos] = useState<VideoItem[]>(INITIAL_VIDEOS);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [visibleCount, setVisibleCount] = useState<number>(
    Math.min(LOAD_STEP, INITIAL_VIDEOS.length || LOAD_STEP)
  );
  ...
  // ✅ 복귀 시 비디오 생성/수정 페이지에서 전달된 썸네일/정보 적용
  useEffect(() => {
    ...
    try {
      if (storedEdit) {
        editParsed = JSON.parse(storedEdit);
      }
    } catch {
      editParsed = null;
    }
  ...
  const handleSave = () => {
    if (!canSave) return;
    const payload = { categoryName, status, videos };
      ...
      <Content>
        <ContentContainer>
          ...
          <Actions>
            ...
            <PrimaryButton type="button" disabled={!canSave} onClick={handleSave}>
              저장하기
            </PrimaryButton>
          </Actions>
        </ContentContainer>
