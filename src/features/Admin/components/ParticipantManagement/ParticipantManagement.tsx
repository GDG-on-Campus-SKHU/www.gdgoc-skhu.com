...
const ParticipantManagement = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        try {
          ...
          const mappedMembers: LocalMember[] = membersData.map((m: Member) => ({
            id: m.memberId,
            school: m.university,
            name: m.name,
        ...
        try {
          ...
          const mappedParticipants: LocalMember[] = participantsData.map((p: Participant) => ({
            id: p.memberId,
            school: p.university,
            name: p.name,
  ...
  const handleToggleMember = async (member: LocalMember) => {
    ...
    try {
      if (isSelected) {
        // 참여자 제거 API 호출
        await removeParticipant(member.id);
